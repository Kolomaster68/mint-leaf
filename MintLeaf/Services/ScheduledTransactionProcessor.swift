...
enum ScheduledTransactionProcessor {
    ...
    /// Only charges for dates within the last 30 days and checks for duplicates.
    @MainActor
    static func processOverdue(context: ModelContext) {
        ...
        for scheduled in overdue {
            ...
            while scheduled.nextDate <= now && scheduled.isActive && iterations < maxIterations {
                ...
                // Only create a transaction if recent AND not already generated
                if scheduled.nextDate >= cutoff, let account = scheduled.account {
                    let alreadyExists = scheduled.generatedTransactions.contains { txn in
                        calendar.isDate(txn.date, inSameDayAs: scheduled.nextDate)
                    }
            ...
            // Save after each subscription so nextDate persists even if app is killed
            try? context.save()
        }
    }
